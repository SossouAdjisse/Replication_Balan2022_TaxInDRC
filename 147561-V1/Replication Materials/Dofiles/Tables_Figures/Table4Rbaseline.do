...
// tmt = treatments : Control, Central, Local,  CLI, CXL ----> added by Sossou
	...
	* Month FE - Compliance - House FE - Condition Exempt
	...
	esttab r11 r21 r31 r41 r51 using "${reploutdir}/main_compliance_results_baseline.tex", ///
	replace label booktabs b(%9.6f) p(%9.6f) booktabs ///
	keep (2.tmt) ///
	order(2.tmt) /// 
...
************
	...
	* Month FE - Revenues - House FE - Condition Exempt
	...
	esttab r12 r22 r32 r42 r52 using "${reploutdir}/main_revenues_results_baseline.tex", ///
	replace label booktabs b(%9.6f) p(%9.6f) booktabs ///
	keep (2.tmt) ///
	order(2.tmt) /// 
	cells("b(fmt(a6))"  "p(fmt(a3) par)" "pvalues(fmt(%9.6f) par([ ]))") ///
