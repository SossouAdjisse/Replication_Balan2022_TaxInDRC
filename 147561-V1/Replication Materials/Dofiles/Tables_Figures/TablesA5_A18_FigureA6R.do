...
*****************************************
	...
	* Tax effects table
		...
		// (1) Normal - Compliance
		eststo r1: reg taxes_paid 2.tmt i.house i.stratum if inlist(tmt,1,2), cl(a7)
		...
		eststo r2: reg taxes_paid 2.tmt i.house i.stratum i.time_FE_tdm_2mo_CvL if inlist(tmt,1,2), cl(a7)
		ritest tmt _b[2.tmt], reps(1000) seed(125) cluster(a7) strata(stratum) nodots: `e(cmdline)'
		matrix pvalues = r(p) 
		mat colnames pvalues = 2.tmt 
		est restore r2
		...
		// (3) Shift two month definition -15/+15 and pick median estimate
			...
			// Regression estimate using p50 version
				preserve
					...
					eststo r3: reg taxes_paid 2.tmt i.house i.stratum i.time_FE_tdm_2mo_CvL if inlist(tmt,1,2), cl(a7)
					ritest tmt _b[2.tmt], reps(1000) seed(125) cluster(a7) strata(stratum) nodots: `e(cmdline)'
					matrix pvalues = r(p) 
					mat colnames pvalues = 2.tmt 
					est restore r3
		...
		restore
		preserve
			eststo r4: xi: GSSUtest taxes_paid t_l i.house i.time_FE_tdm_2mo_CvL i.stratum,vce(robust) cluster(a7)
			su taxes_paid if t_c==1 & time_FE_tdm_2mo_CvL!=.
			estadd local Mean=abs(round(`r(mean)',.001))
			estadd scalar Observations = `e(N)'
		...
		// (5) Month FE
		eststo r5: reg taxes_paid 2.tmt i.house i.stratum i.time_FE_tdm_1mo if inlist(tmt,1,2), cl(a7)
		ritest tmt _b[2.tmt], reps(1000) seed(125) cluster(a7) strata(stratum) nodots: `e(cmdline)'
		matrix pvalues = r(p) 
		mat colnames pvalues = 2.tmt 
		est restore r5
		...
		* Period when both Local and CLI are active
		...
		eststo r6: reg taxes_paid 2.tmt i.house i.stratum if today_alt>=`minimum_cvl' & today_alt<`maximum_cvl' & inlist(tmt,1,2), cl(a7)
		ritest tmt _b[2.tmt], reps(1000) seed(125) cluster(a7) strata(stratum) nodots: `e(cmdline)'
		matrix pvalues = r(p) 
		mat colnames pvalues = 2.tmt 
		est restore r6
		...
		preserve
		restore
		eststo r7: reg taxes_paid 2.tmt i.house i.stratum , cl(a7) // Remote the wight[iw=cem_w]
		ritest tmt _b[2.tmt], reps(1000) seed(125) cluster(a7) strata(stratum) nodots: `e(cmdline)'
		matrix pvalues = r(p) 
		mat colnames pvalues = 2.tmt 
		est restore r7
		...
		esttab r1 r2 r3 r4 r5 r6 r7 using "${reploutdir}/compl_CvL_results_timeimbalR.tex", ///
