...
***********
	...
	* Compliance - Periphery
	eststo r1: reg taxes_paid 2.tmt i.stratum i.time_FE_tdm_2mo_CvL if inlist(tmt,1,2) & house==1, cl(a7)
	...
	* Compliance - MM
	eststo r2: reg taxes_paid 2.tmt i.stratum i.time_FE_tdm_2mo_CvL if inlist(tmt,1,2) & house==2, cl(a7)
	ritest tmt _b[2.tmt], reps(1000) seed(125) cluster(a7) strata(stratum): `e(cmdline)'
	matrix pvalues = r(p) 
	mat colnames pvalues = 2.tmt 
	est restore r2 
	...
	* House Quality - Compliance
	...
	eststo r3: reg hq_y 2.tmt i.house i.stratum i.time_FE_tdm_2mo_CvL a7_hq_y if inlist(tmt,1,2) & taxes_paid==1, cl(a7)
	ritest tmt _b[2.tmt], reps(1000) seed(125) cluster(a7) strata(stratum): `e(cmdline)'
	matrix pvalues = r(p) 
	mat colnames pvalues = 2.tmt 
	est restore r3 
	...
	* Income - Compliance
	...
	eststo r4: reg inc_mo_avg 2.tmt i.house i.stratum i.time_FE_tdm_2mo_CvL a7_inc_mo_avg if inlist(tmt,1,2) & taxes_paid==1, cl(a7)
	ritest tmt _b[2.tmt], reps(1000) seed(125) cluster(a7) strata(stratum): `e(cmdline)'
	matrix pvalues = r(p) 
	mat colnames pvalues = 2.tmt 
	est restore r4 
	...
	* Liquidity - Compliance
	eststo r5: reg econ_ind 2.tmt i.house i.stratum i.time_FE_tdm_2mo_CvL a7_econ_ind if inlist(tmt,1,2) & taxes_paid==1, cl(a7)
	ritest tmt _b[2.tmt], reps(1000) seed(125) cluster(a7) strata(stratum): `e(cmdline)'
	matrix pvalues = r(p) 
	mat colnames pvalues = 2.tmt 
	est restore r5 
	...
	esttab r1 r2 r3 r4 r5 using "${reploutdir}/main_incidence_results9R1_pv.tex", ///
	replace label b(%9.3f) p(%9.3f) booktabs ///
	keep (2.tmt a7_hq_y a7_inc_mo_avg a7_econ_ind) ///
	order(2.tmt) ///
	...
	* CSV Format 
	esttab r1 r2 r3 r4 r5 using "${reploutdir}/main_incidence_results9R1_pv.csv", ///
	replace label b(%9.3f) p(%9.3f) ///
	keep (2.tmt a7_hq_y a7_inc_mo_avg a7_econ_ind) ///
	order(2.tmt) ///
	cells("b(fmt(a3))"  "p(fmt(a3) par)" "pvalues(fmt(%9.3f) par([ ]))") ///
	scalar(Clusters Mean) sfmt(0 3) ///
	mtitles("Paid - Periph" "Paid - MM" "HQ - Visited" "HQ - Paid" "Income - Visited" "Income - Paid") ///
	indicate("Month FE = *2mo*""House FE = *house*""Stratum FE = *stratum*") ///
	star(* 0.10 ** 0.05 *** 0.001) ///
	nogaps nonotes compress
