...
* Latex Output
	esttab r11 r21 r31 r41 r51 using "${reploutdir}/main_compliance_results4R1_pv.tex", ///
...
* CSV  Output 
	esttab r11 r21 r31 r41 r51 using "${reploutdir}/main_compliance_results4R1_pv.csv", ///
...
* Latex Output
	esttab r12 r22 r32 r42 r52 using "${reploutdir}/main_revenues_results4R1_pv.tex", ///
	replace label booktabs b(%9.3f) p(%9.3f) ///
	keep (2.tmt) ///
	order(2.tmt) /// 
	cells("b(fmt(a3))"  "p(fmt(a3) par)" "pvalues(fmt(%9.3f) par([ ]))") ///
 ...
 * CSV Output
	esttab r12 r22 r32 r42 r52 using "${reploutdir}/main_revenues_results4R1z_pv.csv", ///
	replace label b(%9.3f) p(%9.3f) ///
	keep (2.tmt) ///
	order(2.tmt) /// 
	cells("b(fmt(a3))"  "p(fmt(a3) par)" "pvalues(fmt(%9.3f) par([ ]))") ///
