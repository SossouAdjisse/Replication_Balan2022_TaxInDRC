...
**************
	...
	* Normal - Compliance - No house FE
	eststo: reg taxes_paid t_l i.trust_chief i.stratum if inlist(tmt,1,2), cl(a7)
	...
	* Month FE - Compliance - No house FE
	eststo: reg taxes_paid t_l i.trust_chief i.stratum i.time_FE_tdm_2mo_CvL if inlist(tmt,1,2), cl(a7)
...
*/
	...
	* Month FE - Compliance - House FE - Condition Exempt
	eststo: reg taxes_paid t_l i.trust_chief i.stratum i.house i.time_FE_tdm_2mo_CvL if inlist(tmt,1,2) & exempt!=1, cl(a7)
	su taxes_paid if t_c==1 & time_FE_tdm_2mo_CvL!=. & exempt!=1
	estadd local Mean=abs(round(`r(mean)',.001))
	estadd scalar Observations = `e(N)'
	estadd scalar Clusters = `e(N_clust)'
	
	esttab using "${reploutdir}/main_compliance_results4R2_pv.tex", ///
	...
	* CSV Format 
	esttab using "${reploutdir}/main_compliance_results4R2_pv.csv", ///
...
************
	...
	* Normal - Revenues - No house FE
	eststo: reg taxes_paid_amt t_l i.trust_chief i.stratum if inlist(tmt,1,2), cl(a7)
	...
	* Month FE - Revenues - No house FE
	eststo: reg taxes_paid_amt t_l i.trust_chief i.stratum i.time_FE_tdm_2mo_CvL if inlist(tmt,1,2), cl(a7)
...
*/
	...
	* Month FE - Revenues - House FE - Condition Exempt
	eststo: reg taxes_paid_amt t_l i.trust_chief i.stratum i.house i.time_FE_tdm_2mo_CvL if inlist(tmt,1,2) & exempt!=1, cl(a7)
	su taxes_paid_amt if t_c==1 & time_FE_tdm_2mo_CvL!=. & exempt!=1
	estadd local Mean=abs(round(`r(mean)',.001))
	estadd scalar Observations = `e(N)'
	estadd scalar Clusters = `e(N_clust)'
	
	esttab using "${reploutdir}/main_revenues_results4R2_pv.tex", ///
	replace label b(%9.3f) p(%9.3f) booktabs ///
	keep (t_l 2.trust_chief 3.trust_chief 4.trust_chief) ///
	order(t_l 2.trust_chief 3.trust_chief 4.trust_chief) ///
	cells("b(fmt(a3))"  "p(fmt(a3) par)") ///
	...
	* CSV Format 
	esttab using "${reploutdir}/main_revenues_results4R2_pv.csv", ///
	replace label b(%9.3f) p(%9.3f) ///
	keep (t_l 2.trust_chief 3.trust_chief 4.trust_chief) ///
	order(t_l 2.trust_chief 3.trust_chief 4.trust_chief) ///
	cells("b(fmt(a3))"  "p(fmt(a3) par)") ///
