...
***********
	...
	esttab using "${reploutdir}/chiefs_info_payease8R2_pv.tex", ///
...
* CSV Format 
	esttab using "${reploutdir}/chiefs_info_payease8R2_pv.csv", ///
	...
	***********
	...
	esttab using "${reploutdir}/chiefs_info_wtp8R2_pv.tex", ///
	replace label b(%9.3f) p(%9.3f) booktabs ///
...
* CSV Format 
	esttab using "${reploutdir}/chiefs_info_wtp8R2_pv.csv", ///
	replace label b(%9.3f) p(%9.3f) ///
	keep (willingness p_willingness walls_final roof_final ravine_final) ///
	order(willingness p_willingness walls_final roof_final ravine_final) ///
	cells("b(fmt(a3))"  "p(fmt(a3) par)") ///
