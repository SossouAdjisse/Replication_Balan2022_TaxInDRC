...
drop _merge
	...
	* Visits - Extensive CvL
	eststo r1: reg visit_post_carto t_l i.house i.stratum i.time_FE_tdm_2mo_CvL if inlist(tmt,1,2) & trust_chief != ., cl(a7)
	...
	* Visits - Intensive CvL
	eststo r2: reg nb_visit_post_carto t_l  i.house i.stratum i.time_FE_tdm_2mo_CvL if inlist(tmt,1,2) & trust_chief != ., cl(a7)
	...
	* Visits Other Contact - Extensive CvL
	eststo r3: reg visits_other_dummy t_l  i.house i.stratum i.time_FE_tdm_2mo_CvL if inlist(tmt,1,2) & trust_chief != ., cl(a7)
	...
	* Visits Other Contact - Intensive CvL
	eststo r4: reg visits_other_nb t_l  i.house i.stratum i.time_FE_tdm_2mo_CvL if inlist(tmt,1,2) & trust_chief != ., cl(a7)
	su visits_other_nb if t_c==1 & time_FE_tdm_2mo_CvL!=.
	estadd local Mean=abs(round(`r(mean)',.001))
	estadd scalar Observations = `e(N)'
	estadd scalar Clusters = `e(N_clust)'
	
	esttab r1 r2 r3 r4 using "${reploutdir}/main_visits_results6R4_pv.tex", ///
...
* CSV Format 
	esttab r1 r2 r3 r4 using "${reploutdir}/main_visits_results6R4_pv.csv", ///
