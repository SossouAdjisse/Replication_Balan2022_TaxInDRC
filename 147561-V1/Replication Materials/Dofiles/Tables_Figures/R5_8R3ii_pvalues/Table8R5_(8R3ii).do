...
***********
	...
	*/
	...
	lab var roof_final "Roof quality"
		...
		eststo: reg visit_post_carto pay_ease walls_final roof_final ravine_final $chief_chars i.house i.stratum if t_cli==1,cluster(a7)
			estadd scalar Clusters = `e(N_clust)'
			sum visit_post_carto if t_cli==1 & pay_ease!=.
			estadd local Mean=abs(round(`r(mean)',.001))
			estadd scalar Observations = `e(N)'
			estadd local ChiefChars "Yes"
		eststo: reg taxes_paid pay_ease  walls_final roof_final ravine_final $chief_chars i.house i.stratum if t_cli==1,cluster(a7)
			estadd scalar Clusters = `e(N_clust)'
		...
		esttab using "${reploutdir}/chiefs_info_payease8R5_pv.tex", ///
		...
		indicate("Chiefs Chars Control = *$chief_chars*""House FE = *house*""Stratum FE = *stratum*") ///
...
***********
		...
		eststo: reg visit_post_carto willingness walls_final roof_final ravine_final $chief_chars i.house i.stratum if t_cli==1,cluster(a7)
			estadd scalar Clusters = `e(N_clust)'
			sum visit_post_carto if t_cli==1 & willingness!=.
			estadd local Mean=abs(round(`r(mean)',.001))
			estadd scalar Observations = `e(N)'
		eststo: reg taxes_paid willingness walls_final roof_final ravine_final $chief_chars i.house i.stratum if t_cli==1,cluster(a7)
			estadd scalar Clusters = `e(N_clust)'
		...
		esttab using "${reploutdir}/chiefs_info_wtp8R5_pv.tex", ///
		replace label b(%9.3f) p(%9.3f) booktabs ///
