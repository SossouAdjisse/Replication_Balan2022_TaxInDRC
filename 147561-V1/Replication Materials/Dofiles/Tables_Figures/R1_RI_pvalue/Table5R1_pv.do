...
di "SE: `se'"
local p1 = round(2*ttail(e(df_r), abs(_b[2.treatment]/_se[2.treatment])),.001)
di "p-value: `p'"
...
di "Central mean:`centralmean'"
	...
	if `counter' == 1 { 
		mat input reg = (`beta', `se', `p1', `p', `r2',`obs',`centralmean') 
		mat rownames reg = `depvar' 
		mat colnames reg = beta SE p1 p r2 N centralmean	 
	}
	
	if `counter' > 1 { 
		mat input reg`counter' = (`beta', `se', `p1', `p',`r2',`obs',`centralmean') 
		mat rownames reg`counter' = `depvar' 
		mat colnames reg`counter' = beta SE p1 p r2 N centralmean	 
	...
	cd "$reploutdir"
	mmat2tex reg using "assessment_bribes5R1_pv.tex", replace  ///
	colnames(beta SE p1 p r2 N centralmean) ///
...
di "Central mean:`centralmean'"
	...
	cd "$reploutdir"
	mmat2tex reg using "attitudes5R1_pv.tex", replace  ///
	colnames(beta SE p1 p r2 N centralmean) ///
