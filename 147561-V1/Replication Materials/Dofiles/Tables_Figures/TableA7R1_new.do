...
***********
	...
	* Month FE - Compliance - House FE - Age, Age-Squared, Gender
	eststo: reg taxes_paid t_l i.trust_chief i.stratum i.house i.time_FE_tdm_2mo_CvL ///
	...
	* Month FE - Compliance - House FE - + Pilot Polygons
	eststo: reg taxes_paid t_l i.trust_chief i.stratum i.house i.time_FE_tdm_2mo_CvL if inlist(tmt,1,2) & inlist(pilot,0,1), cl(a7)
	...
	* Month FE - Compliance - House FE - + Drop Polygon 654
	eststo: reg taxes_paid t_l i.trust_chief i.stratum i.house i.time_FE_tdm_2mo_CvL if inlist(tmt,1,2) & pilot==0 & a7!=654, cl(a7)
	...
	* Month FE - Compliance - House FE - + Winsorize (at polygon level)
	preserve
		...
		collapse (mean) taxes_paid trust_chief_mean = trust_chief (min) time_FE_tdm_2mo_CvL (max) t_l t_c stratum,by(a7 tmt)
		winsor taxes_paid,gen(taxes_paid_w10) p(0.1) highonly
		eststo: reg taxes_paid_w10 t_l trust_chief_mean i.stratum i.time_FE_tdm_2mo_CvL if inlist(tmt,1,2), robust
	...
	esttab using "${reploutdir}/compl_results_controlsR1_new.tex", ///
...
***********
	...
	* Month FE - Revenues - House FE - Age, Age-Squared, Gender
	eststo: reg taxes_paid_amt t_l i.trust_chief i.stratum i.house i.time_FE_tdm_2mo_CvL ///
		age_prop age_prop_squared sex_prop educ_yrs_prop ///
		age_prop_mis age_prop_squared_mis sex_prop_mis educ_yrs_prop_mis if inlist(tmt,1,2) & pilot==0, cl(a7)
	...
	* Month FE - Revenues - House FE - + Pilot Polygons
	eststo: reg taxes_paid_amt t_l i.trust_chief i.stratum i.house i.time_FE_tdm_2mo_CvL if inlist(tmt,1,2) & inlist(pilot,0,1), cl(a7)
	...
	* Month FE - Revenues - House FE - + Drop Polygon 654
	eststo: reg taxes_paid_amt t_l i.trust_chief i.stratum i.house i.time_FE_tdm_2mo_CvL if inlist(tmt,1,2) & pilot==0 & a7!=654, cl(a7)
	...
	* Month FE - Revenues - House FE - + Winsorize (at polygon level)
	preserve
		drop if time_FE_tdm_2mo_CvL==.
		collapse (mean) taxes_paid_amt trust_chief_mean = trust_chief (min) time_FE_tdm_2mo_CvL (max) t_l t_c stratum,by(a7 tmt)
		winsor taxes_paid_amt,gen(taxes_paid_amt_w10) p(0.1) highonly
		eststo: reg taxes_paid_amt_w10 t_l trust_chief_mean i.stratum i.time_FE_tdm_2mo_CvL if inlist(tmt,1,2), robust
	...
	esttab using "${reploutdir}/rev_results_controlsR1_new.tex", ///
	replace label b(%9.3f) p(%9.3f) booktabs ///
	keep (t_l 2.trust_chief 3.trust_chief 4.trust_chief trust_chief_mean) ///
	order(t_l 2.trust_chief 3.trust_chief 4.trust_chief trust_chief_mean) ///
	scalar(Clusters Mean) sfmt(0 3 3) ///
