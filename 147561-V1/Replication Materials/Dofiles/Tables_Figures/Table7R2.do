...
***********
	...
	* Month FE - Compliance
	eststo r1: reg taxes_paid t_cli i.trust_chief i.stratum i.time_FE_tdm_2mo_CvCLI if inlist(tmt,1,3), cl(a7)
	...
	* Month FE - Revenues
	eststo r2: reg taxes_paid_amt t_cli i.trust_chief i.stratum i.time_FE_tdm_2mo_CvCLI if inlist(tmt,1,3), cl(a7)
	...
	* Month FE - Visited
	eststo r3: reg visit_post_carto t_cli i.trust_chief i.stratum i.time_FE_tdm_2mo_CvCLI if inlist(tmt,1,3), cl(a7)
	...
	* Month FE - Visits
	eststo r4: reg nb_visit_post_carto t_cli i.trust_chief i.stratum i.time_FE_tdm_2mo_CvCLI if inlist(tmt,1,3), cl(a7)
	...
	* Month FE - Compliance Conditional on Visited
	eststo r5: reg taxes_paid t_cli i.trust_chief i.stratum i.time_FE_tdm_2mo_CvCLI if inlist(tmt,1,3) & visit_post_carto==1, cl(a7)
	...
	* Month FE - Compliance CvLvCLI
	eststo r6: reg taxes_paid t_cli t_l i.trust_chief i.stratum i.time_FE_tdm_2mo_CvLvCLI if inlist(tmt,1,2,3), cl(a7)
	test t_cli = t_l
	local p_CLIvC = `r(p)'
	...
	eststo r7: reg taxes_paid_amt t_cli t_l i.trust_chief i.stratum i.time_FE_tdm_2mo_CvLvCLI if inlist(tmt,1,2,3), cl(a7)
	test t_cli = t_l
	local p_CLIvC = `r(p)'
	su taxes_paid if t_c==1 & time_FE_tdm_2mo_CvLvCLI!=.
	estadd local Mean=abs(round(`r(mean)',.001))
	...
	esttab r1 r2 r3 r4 r5 r6 r7 using "${reploutdir}/main_centralwinfo_resultsR2.tex", ///
	...
	indicate("Time FE = *2mo*""Stratum FE = *stratum*") ///
