...
***********
	...
	ritest p_pay_ease _b[p_pay_ease], reps(1000) seed(125) strata(stratum): `e(cmdline)' 
		...
		estadd scalar Clusters = `e(N_clust)'
		sum visit_post_carto if t_cli==1 & p_pay_ease!=. & walls_final!=. & roof_final!=. & ravine_final!=.
		estadd local Mean=abs(round(`r(mean)',.001))
		estadd scalar Observations = `e(N)'
	...
	ritest p_pay_ease _b[p_pay_ease], reps(1000) seed(125) strata(stratum): `e(cmdline)' 
		...
		estadd scalar Clusters = `e(N_clust)'
		sum taxes_paid if t_cli==1 & p_pay_ease!=. & walls_final!=. & roof_final!=. & ravine_final!=.
		estadd local Mean=abs(round(`r(mean)',.001))
		estadd scalar Observations = `e(N)'
	...
	esttab r1 r2 r3 r4 r5 r6 r7 r8 using "${reploutdir}/chiefs_info_payeaseLC_R1.tex", ///
	...
	***********
	...
	eststo r1: reg visit_post_carto willingness i.house i.stratum if t_cli==1,cluster(a7)
	ritest willingness _b[willingness], reps(1000) seed(125) strata(stratum): `e(cmdline)' 
		matrix pvalues = r(p) 
		...
		estadd local Mean=abs(round(`r(mean)',.001))
		estadd scalar Observations = `e(N)'
	eststo r2: reg taxes_paid willingness i.house i.stratum if t_cli==1,cluster(a7)
	ritest willingness _b[willingness], reps(1000) seed(125) strata(stratum): `e(cmdline)' 
		matrix pvalues = r(p) 
		mat colnames pvalues = willingness  
		est restore r2
	...
	eststo r3: reg visit_post_carto willingness walls_final roof_final ravine_final i.house i.stratum if t_cli==1,cluster(a7)
	ritest willingness _b[willingness], reps(1000) seed(125) strata(stratum): `e(cmdline)' 
		matrix pvalues = r(p) 
		mat colnames pvalues = willingness  
		est restore r3 
		...
		estadd local Mean=abs(round(`r(mean)',.001))
		estadd scalar Observations = `e(N)'
	eststo r4: reg taxes_paid willingness walls_final roof_final ravine_final i.house i.stratum if t_cli==1,cluster(a7)
	ritest willingness _b[willingness], reps(1000) seed(125) strata(stratum): `e(cmdline)' 
		matrix pvalues = r(p) 
		mat colnames pvalues = willingness  
		est restore r4
	...
	eststo r5: reg visit_post_carto p_willingness  walls_final roof_final ravine_final i.house i.stratum if t_l==1,cluster(a7)
	ritest p_willingness _b[p_willingness], reps(1000) seed(125) strata(stratum): `e(cmdline)' 
		matrix pvalues = r(p) 
		...
		estadd local Mean=abs(round(`r(mean)',.001))
		estadd scalar Observations = `e(N)'
	eststo r6: reg taxes_paid p_willingness  walls_final roof_final ravine_final i.house i.stratum if t_l==1,cluster(a7)
	ritest p_willingness _b[p_willingness], reps(1000) seed(125) strata(stratum): `e(cmdline)' 
		matrix pvalues = r(p) 
		mat colnames pvalues = p_willingness  
		est restore r6
		...
		estadd local Mean=abs(round(`r(mean)',.001))
		estadd scalar Observations = `e(N)'
	eststo r7: reg visit_post_carto p_willingness walls_final roof_final ravine_final i.house i.stratum if t_c==1,cluster(a7)
	ritest p_willingness _b[p_willingness], reps(1000) seed(125) strata(stratum): `e(cmdline)' 
		matrix pvalues = r(p) 
		mat colnames pvalues = p_willingness  
		est restore r7 
		...
		estadd local Mean=abs(round(`r(mean)',.001))
		estadd scalar Observations = `e(N)'
	eststo r8: reg taxes_paid p_willingness walls_final roof_final ravine_final i.house i.stratum if t_c==1,cluster(a7)
	ritest p_willingness _b[p_willingness], reps(1000) seed(125) strata(stratum): `e(cmdline)' 
		matrix pvalues = r(p) 
		mat colnames pvalues = p_willingness  
		est restore r8 
	...
	esttab r1 r2 r3 r4 r5 r6 r7 r8 using "${reploutdir}/chiefs_info_wtpLC_R1.tex", ///
	replace label b(%9.3f) p(%9.3f) booktabs ///
