***********
* Table 4 *
* This table replicates Table 4, but restricted only-baseline sample,and using randomized inference. 
***********
...
*######################################
	...
	* keep if tmt==1 | tmt==2 | tmt==3 // Commented by Sossou
	keep if baseline == 1 // Restrict sample to only-baseline. 
...
// tmt = treatments : Control, Central, Local,  CLI, CXL ----> added by Sossou
	...
	* Month FE - Compliance - House FE - Condition Exempt
	...
	esttab r11 r21 r31 r41 r51 using "${reploutdir}/main_compliance_results4R12.tex", ///
	replace label booktabs b(%9.6f) p(%9.6f) ///
	keep (2.tmt) ///
	order(2.tmt) /// 
...
************
	...
	* Month FE - Revenues - House FE - Condition Exempt
	...
	esttab r12 r22 r32 r42 r52 using "${reploutdir}/main_revenues_results4R12.tex", ///
	replace label booktabs b(%9.6f) p(%9.6f) ///
	keep (2.tmt) ///
	order(2.tmt) /// 
	cells("b(fmt(a6))"  "p(fmt(a3) par)" "pvalues(fmt(%9.6f) par([ ]))") ///
